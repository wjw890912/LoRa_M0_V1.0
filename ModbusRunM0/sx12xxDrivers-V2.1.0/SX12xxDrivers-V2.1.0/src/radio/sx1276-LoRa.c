...
数据速率计算公式
 DR(bps)= SF*( BW/2^SF)*CR
  BW=SignalBw
  SF=SpreadingFactor 
  CR=ErrorCoding

  例如：SignalBw =500K,	SpreadingFactor =12,ErrorCoding=4/5
  那么此时的速率是：

  DR = 12*(500000/2^12)*(4/5) = 12*(500000/4096)*0.8 =1171.8 

  那么当前的速率是 1.171bps 也就是说每秒钟1171个位。转换为字节 1172/8 =146B/S
...
tLoRaSettings LoRaSettings =
{
    ...
    true,             // CrcOn [0: OFF, 1: ON]	CRC
    false,            // ImplicitHeaderOn [0: OFF, 1: ON]	  关闭头部
    0,                // RxSingleOn [0: Continuous, 1 Single] 采用连续接收模式
    0,                // FreqHopOn [0: OFF, 1: ON]	调频关闭
...
/*
  ...
  //默认的配置 约 18229 bps 距离和4432差不多，,900米左右 8ms发送完毕
...
uint32_t SX1276LoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_RUNNING:
          SX1276Read( REG_LR_IRQFLAGS, &rxstate );//读取状态IRQ	的TxDone
        if( /*DIO0 == 1 */((rxstate&0x40)==0x40)) // RxDone
        {
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
        ...
        if( /*DIO2 == 1*/((rxstate&0x02)==0x02) ) // FHSS Changed Channel
        {
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
    ...
    case RFLR_STATE_TX_RUNNING:
	  ...
	  SX1276Read( REG_LR_IRQFLAGS, &txstate );//读取状态IRQ	的TxDone
        if( /*DIO0 == 1*/ ((txstate&0x08)==0x08)) // TxDone
        {
            // Clear Irq
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_TXDONE  );
            RFLRState = RFLR_STATE_TX_DONE;   
        }
        if( /*DIO2 == 1*/ ((txstate&0x02)==0x02)) // FHSS Changed Channel
        {
            if( LoRaSettings.FreqHopOn == true )
            {
                SX1276Read( REG_LR_HOPCHANNEL, &SX1276LR->RegHopChannel );
